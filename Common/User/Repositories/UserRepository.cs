...
namespace ShareNet.Common.User.Repositories
{
    ...
    {
        public ISettingsManager<UserSettings> userSettingsManager = DIContainer.Resolve<ISettingsManager<UserSettings>>();
        ...
        /// </summary>
        /// <param name="noModeratedUserPoint">用户自动解除管制状态所需的积分（用户综合积分）</param>
        ...
        public void UpdateAvatar(IUser user,long dateAvatar, string avatar)
        {
            if (user == null)
                return;

            var sql_Update = PetaPoco.Sql.Builder;
            sql_Update.Append("update tn_Users set Avatar=@0, DateAvatar=@1 where UserId = @2", avatar,user.DateAvatar,user.UserId);
            int affectedCount = CreateDao().Execute(sql_Update);

            User updateUser = user as User;
            updateUser.Avatar = avatar;
            updateUser.DateAvatar = dateAvatar;
            base.OnUpdated(updateUser);
        }
