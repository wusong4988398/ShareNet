...
namespace PetaPoco.Core
{
    public class PocoData
    {
        ...
        public Delegate GetFactory(string sql, string connectionString, int firstColumn, int countColumns, IDataReader reader, IMapper defaultMapper)
        {
            ...
            {
                // Create the method
                var m = new DynamicMethod("petapoco_factory_" + PocoFactories.Count.ToString(), this.type, new Type[] { typeof(IDataReader) }, true);
                var il = m.GetILGenerator();
                var mapper = Mappers.GetMapper(this.type, defaultMapper);
                ...
                else if (this.type.IsValueType || this.type == typeof(string) || this.type == typeof(byte[]))
                {
                    ...
                    var converter = GetConverter(mapper, null, srcType, this.type);
                ...
                {
                    // var poco=new T()
                    var ctor = this.type.GetConstructor(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null, new Type[0], null);
                    if (ctor == null)
                        throw new InvalidOperationException("Type [" + this.type.FullName + "] should have default public or non-public constructor");
                    ...
                    var fnOnLoaded = RecurseInheritedTypes<MethodInfo>(this.type,
                ...
                // Cache it, return it
                return m.CreateDelegate(Expression.GetFuncType(typeof(IDataReader), this.type));
            }
                );
        }
