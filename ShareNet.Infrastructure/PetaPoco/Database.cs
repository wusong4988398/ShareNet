...
namespace PetaPoco
{
    ...
    public class Database :IDisposable, IDatabase
    {
        ...
        ///  执行sql取出第一列记录(带参数)
        /// </summary>
        /// <param name="sql">一个SQL builder对象表示一个查询和一个参数</param>
        /// <param name="args">任何sql参数</param>
        /// <returns></returns>
        ...
        public IEnumerable<T> FetchByPrimaryKeys<T>(IEnumerable<object> primaryKeys)
        {
            ...
            string str = this._provider.EscapeSqlIdentifier(PocoData.ForType(typeof(T)).TableInfo.PrimaryKey);
        ...
        public PagingEntityIdCollection FetchPagingPrimaryKeys<TEntity>(long maxRecords, int pageSize, int pageIndex,
            Sql sql) where TEntity : IEntity
        {
            string str2;
            string str3;
            string sQL = sql.SQL;
            ...
            return new PagingEntityIdCollection(this.FetchFirstColumn(str3,arguments).ToList(),this.ExecuteScalar<long>(str2,arguments));
...

        {
            ...
            {
                sql = AutoSelectHelper.AddSelectClause<T>(this._provider, sql, primaryKey);
            }
        ...
        protected void BuildPagingPrimaryKeyQueries(long maxRecords, long skip, long take, string primaryKey, string sql, ref object[] args, out string sqlCount, out string sqlPage)
        {
            ...
            sqlPage = this._provider.BuildPageQuery(skip, take, parts, ref args, primaryKey);
        }
        ...
        protected bool SplitSqlForPagingOptimized(long maxRecords, string sql, string primaryKey, out string sqlCount, out string sqlSelectRemoved, out string sqlOrderBy)
        {
            ...
            sqlCount = null;
            sqlOrderBy = null;
            var helper = new PagingHelper();
            Match match = helper.RegexColumns.Match(sql);
            if (!match.Success)
            {
                return false;
            }
            ...
            if (helper.RegexDistinct.IsMatch(sqlSelectRemoved))
            {
            ...
            match = helper.RegexOrderBy.Match(sqlCount);
            if (!match.Success)
            {
                sqlOrderBy = null;
            }
